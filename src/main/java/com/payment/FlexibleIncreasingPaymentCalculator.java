...
/**
 * Esnek Artan Taksitli Ödeme Hesaplayıcı.
 * Taksit tutarı belirli aralıklarla miktar veya oran bazlı artırılır.
 * BigDecimal + ROUND_HALF_UP(2 digit) kullanır.
 */
public class FlexibleIncreasingPaymentCalculator {
    ...
    // ─── Request modeli ───
    public static class IncreasingLoanRequest {
        public LoanType loanType;
        public BigDecimal amount;
        public int maturity;
        public BigDecimal interestRate; // ör: 4.15
        public PaymentFrequency paymentFrequency;
        public int increaseStartInstallmentNo;
        public int increaseFrequency;
        public BigDecimal increaseAmount; // miktar bazlı (nullable)
        public BigDecimal increaseRate; // oran bazlı (nullable, ör: 0.05)
    }
    ...
    /**
     * Artış noktası mı kontrol eder.
     * paymentNumber taksit numarası (1-based, sadece ödeme yapılan aylar sayılır).
     */
    private static boolean isIncreasePoint(int paymentNumber, int start, int frequency) {
        return paymentNumber >= start && (paymentNumber - start) % frequency == 0;
    }

    // ─── Ana hesaplama ───

    public CalculationResult calculate(IncreasingLoanRequest req) {
        ...
        BigDecimal baseInstallment = findInstallment(
                ...
                kkdfRate, bsmvRate,
                req.increaseStartInstallmentNo,
                req.increaseFrequency,
                req.increaseAmount,
                req.increaseRate,
                payInterval);
    ...
    private BigDecimal findInstallment(
            ...
            int incStart, int incFreq,
            BigDecimal incAmount, BigDecimal incRate,
            int payInterval) {
        ...
        for (int iter = 0; iter < 300; iter++) {
            ...
            BigDecimal finalBalance = simulateBalance(
                    ...
                    kkdfRate, bsmvRate,
                    incStart, incFreq, incAmount, incRate,
                    mid, payInterval);
        ...
        BigDecimal balDown = simulateBalance(principal, monthlyRate, maturity,
                kkdfRate, bsmvRate, incStart, incFreq, incAmount, incRate,
    ...
    private BigDecimal simulateBalance(
        ...
        for (int i = 1; i <= maturity; i++) {
            ...
            if (!isPaymentMonth(i, payInterval)) {
                ...
                // Artış noktası kontrolü
                if (isIncreasePoint(paymentCount, incStart, incFreq)) {
                    if (incRate != null) {
                        currentInstallment = round(currentInstallment.multiply(
                                BigDecimal.ONE.add(incRate)));
                    } else if (incAmount != null) {
                        currentInstallment = round(currentInstallment.add(incAmount));
                    }
                }
